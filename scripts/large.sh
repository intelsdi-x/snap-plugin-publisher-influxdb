...
__proj_name="snap-plugin-publisher-influxdb"
...
_docker_project docker-compose exec -T golang gvm-bash.sh "gvm use $GOLANGVER; export INFLUXDB_VERSION=$INFLUXDB_VERSION; mkdir -p ${_docker_org_path}; cp -Rf /${__proj_name} ${_docker_org_path}; (cd ${_docker_proj_path} && ./scripts/large_jenkins.sh)"
