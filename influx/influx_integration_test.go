	...
	Convey("snap plugin InfluxDB integration testing with Influx", t, func() {
		...
		Convey("Publish integer metric", func() {
			metrics := []plugin.MetricType{
				*plugin.NewMetricType(core.NewNamespace([]string{"foo"}), time.Now(), tags, "some unit", 99),
			}
			buf.Reset()
		...
		Convey("Publish float metric", func() {
			metrics := []plugin.MetricType{
				*plugin.NewMetricType(core.NewNamespace([]string{"bar"}), time.Now(), tags, "some unit", 3.141),
			}
			buf.Reset()
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
		...
		Convey("Publish string metric", func() {
			metrics := []plugin.MetricType{
				*plugin.NewMetricType(core.NewNamespace([]string{"qux"}), time.Now(), tags, "some unit", "bar"),
			}
			buf.Reset()
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
		...
		Convey("Publish boolean metric", func() {
			metrics := []plugin.MetricType{
				*plugin.NewMetricType(core.NewNamespace([]string{"baz"}), time.Now(), tags, "some unit", true),
			}
			buf.Reset()
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
		...
		Convey("Publish multiple metrics", func() {
			metrics := []plugin.MetricType{
				*plugin.NewMetricType(core.NewNamespace([]string{"foo"}), time.Now(), tags, "some unit", 101),
				*plugin.NewMetricType(core.NewNamespace([]string{"bar"}), time.Now(), tags, "some unit", 5.789),
			}
			buf.Reset()
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
